...
def update_building_leg_headline(cursor, path, type):
    ...
    cursor.execute(
        "CREATE INDEX `idx_site_loc` ON `fs_bds`.`building_leg_headline_new` (site_loc) COMMENT '' ALGORITHM DEFAULT LOCK NONE")

    logger.info(f"create idx_sigungu_code...")
    cursor.execute(
        "CREATE INDEX `idx_sigungu_code` ON `fs_bds`.`building_leg_headline_new` (sigungu_code) COMMENT '' ALGORITHM DEFAULT LOCK NONE")

    logger.info(f"create idx_land_area...")
    cursor.execute(
        "CREATE INDEX `idx_land_area` ON `fs_bds`.`building_leg_headline_new` (land_area) COMMENT '' ALGORITHM DEFAULT LOCK NONE")

    logger.info(f"create idx_total_floor_area...")
    cursor.execute(
        "CREATE INDEX `idx_total_floor_area` ON `fs_bds`.`building_leg_headline_new` (total_floor_area) COMMENT '' ALGORITHM DEFAULT LOCK NONE")
...
def update_building_floor_info(cursor, path, type):
    ...
    logger.info(f"create idx_bunji...")
    cursor.execute(
        "CREATE INDEX `idx_bun_ji` ON `fs_bds`.`building_floor_info_new` (bun, ji, leg_dong_code_val) COMMENT '' ALGORITHM DEFAULT LOCK NONE")
...
def update_building_sub_addr(cursor, path, type):
    logger.info(f"updating {type} data")
    ...
    logger.info(f"make leg_dong_code_val...")
    cursor.execute(
        "UPDATE fs_bds.building_sub_addr_new SET leg_dong_code_val = concat(sigungu_code, leg_dong_code)")
    logger.info(f"make sub_leg_dong_code_val...")
    cursor.execute(
        "UPDATE fs_bds.building_sub_addr_new SET sub_leg_dong_code_val = concat(sub_sigungu_code, sub_leg_dong_code)")

    logger.info(f"create idx_building_id...")
    cursor.execute(
        "CREATE INDEX `idx_building_id` ON `fs_bds`.`building_sub_addr_new` (building_id) COMMENT '' ALGORITHM DEFAULT LOCK NONE")

    logger.info(f"create idx_bunji...")
    cursor.execute(
        "CREATE INDEX `idx_bun_ji` ON `fs_bds`.`building_sub_addr_new` (bun, ji, leg_dong_code_val) COMMENT '' ALGORITHM DEFAULT LOCK NONE")
    ...
    change_as_new_table(cursor, "building_sub_addr")
...
def update_search_table(cursor):
    ...
    drop_table_if_exist(cursor, "address_search_new")
    create_new_table(cursor, "address_search")

    cursor.execute(
    """
        INSERT INTO address_search_new (
        id,
        jibun,
        road,
        building_name
    ...
    logger.info(f"create idx_jibun...")
    cursor.execute("CREATE INDEX `idx_jibun` ON `fs_bds`.`address_search_new` (key_jibun) COMMENT '' ALGORITHM DEFAULT LOCK NONE")
    logger.info(f"create idx_road...")
    cursor.execute("CREATE INDEX `idx_road` ON `fs_bds`.`address_search_new` (key_road) COMMENT '' ALGORITHM DEFAULT LOCK NONE")
    logger.info(f"create idx_building...")
    cursor.execute("CREATE INDEX `idx_building` ON `fs_bds`.`address_search_new` (key_building) COMMENT '' ALGORITHM DEFAULT LOCK NONE")            

    change_as_new_table(cursor, "address_search")
