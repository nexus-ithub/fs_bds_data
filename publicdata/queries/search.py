...
CREATE_SEARCH_NEW = """
    CREATE TABLE `address_search_new` (
        `id` varchar(19) NOT NULL,
        `jibun` varchar(150) NOT NULL,
        `road` varchar(150) NOT NULL,
        `building_name` varchar(128) NOT NULL,
        `key_jibun` varchar(150) GENERATED ALWAYS AS (replace(replace(lcase(`jibun`),' ',''),'-','')) STORED,
        `key_road` varchar(150) GENERATED ALWAYS AS (replace(replace(lcase(`road`),' ',''),'-','')) STORED,
        `key_building` varchar(128) GENERATED ALWAYS AS (replace(replace(lcase(`building_name`),' ',''),'-','')) STORED,
